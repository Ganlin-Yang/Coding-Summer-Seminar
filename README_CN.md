...
- ##### 编解码：
  ...
  对$\hat{y}$的编解码使用torchac编码器。编码提供了并行编码和串行编码两种方式，解码只有串行解码一种方式。(需要注意的一点时串行编码并不是一个好方案，我们目前把它注释掉并且没有做修改)。在编码时，我们首先对$\hat{y}$进行维度转换以组织码流，解码时利用逐渐更新的累积概率表依次从码流中解码出$\hat{y}$的切片，并最终拼接，过网络得到重建图像。
...
我们Factorized、Hyperprior、JointAutoregressive三种模型以及JPEG、JPEG2000、compressai的R-D Curve如下所示：

![R-D_Curve_full](statistics/R-D_Curve_new.jpg)

可以看到，我们的Factorized、Hyperprior两种模型的性能均远超JPEG。同时，我们Hyperprior模型的性能可以超过JPEG2000。但由于训练方式以及其他原因，我们的JointAutoregressive模型未达到预期的性能。与compressai官方数据对比可知，我们的Factorized、Hyperprior两种模型在相同bpp的情况下，PSNR与compressai官方数据相差约1dB。
...
上图的第一排从左到右分别为kodak-19原图、Factorized模型、Hyperprior模型的重建图像，第二排分别为JointAutoregressive模型与JPEG的重建图像。具体的bpp与psnr如下：
...
在bpp相近的情况下，我们的Factorized和Hyperprior两种模型整体上都具有较高的主观恢复质量。但对于原图的一些细节部分，比如天空和栅栏的纹理，我们的Factorized模型恢复的较为模糊，Hyperprior模型和JointAutoregressive模型保留了一些纹理，但仍有少许模糊存在。JPEG在这一码率点则出现了较为严重的块状失真。
