...
def sum_dict(all_result,one_pic_result,pic_num):
    for i in range(4):
...
def plot_RDCurve():
    ...
    bpp, psnr, _, _=file_get("Factorized")
    plt.plot(np.array(bpp), np.array(psnr), label="factorized", marker='o', color='red')

    bpp, psnr, _, _=file_get("Hyperprior")
    plt.plot(np.array(bpp), np.array(psnr), label="hyperprior", marker='o', color='blue')
    
    # 两种z_hat获取方式对比
    # bpp, psnr, _, _=file_get("hyperprior1")
    # plt.plot(np.array(bpp), np.array(psnr), label="decompress_enc_time", marker='x', markersize=10, color='red')  # markersize=10, 

    # bpp, psnr, _, _=file_get("hyperprior2")
    # plt.plot(np.array(bpp), np.array(psnr), label="round_enc_time", marker='o', markersize=5, color='blue')  # markersize=5, 
    ...
    # # hyper, lmbda=0.0067
    # plt.plot(np.array(hyper_lmbda1_bpp), np.array(hyper_lmbda1_psnr), label="hyper_lmbda1", marker='o', markersize=1, color='green')
